/* Render named sheets to preview/ as PNG, for review without opening the PDF.
   ...
   node tools/preview.mjs                     the default representative set
   node tools/preview.mjs "Cover" "S1-14 key" specific sheets */
...
const want = process.argv.length > 2 ? process.argv.slice(2) : DEFAULT;
...
for (const label of want) {
  ...
  const file = resolve(root, `preview/p${n}-${slug}.png`);
